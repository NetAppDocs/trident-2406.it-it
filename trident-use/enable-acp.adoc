...
. (Solo registri personalizzati) Segui questi passaggi per spostare l'immagine nel tuo registro personalizzato. Se non stai utilizzando un registro, segui i passaggi dell'operatore Trident in <<first-step,sezione successiva>>.
+
...
. [[primo-passo]]Abilita Astra Control Provisioner in Astra Trident utilizzando il metodo di installazione che hai utilizzato in precedenza:
+
[role="tabbed-block"]
====
