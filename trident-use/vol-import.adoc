...
. Utilizza il `tridentctl import` comando per specificare il nome del back-end di Astra Trident che contiene il volume e il nome che identifica in modo univoco il volume sullo storage (ad esempio: ONTAP FlexVol, Element Volume, Cloud Volumes Service path). L' `-f`argomento è necessario per specificare il percorso del file PVC.
+
[listing]
----
